...
-- take an automaton -> returns a list of transition rules in that automaton
getDelta :: (Eq st, Eq sy) => Automaton st sy -> [(st,sy,st)]
getDelta (states, syms, i, f, delta) = delta

-- takes an automaton -> returns a list of goal configurations for that automaton
getGoalConfigs :: (Eq st, Eq sy) => Automaton st sy -> [(st,[sy])]
getGoalConfigs (states, syms, i, f, delta) = map (\x -> (x, [])) f

-- executes the parser transition consume
-- takes a list of transition rules for an FSA and a current parser configuration (type (st,[sy])), and it returns the list of possible next configurations that the parser could be in after taking a single consume step
consumeFSA :: (Eq st, Eq sy) 
                => [(st,sy,st)] -- delta
